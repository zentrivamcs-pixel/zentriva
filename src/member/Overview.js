...
import { currentMember, quickBenefits, recentActivity, upcomingEvent } from './memberData';

function Overview() {
  const qrValue = useMemo(() => buildMembershipQrValue(currentMember), []);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="font-body-lg text-body-lg text-secondary mt-1">
              Your membership is active and secure. Here is your daily overview.
            </p>
          </div>
          <div className="flex gap-3">
            <button
              type="button"
              className="bg-primary text-on-primary px-6 py-3 font-label-md text-label-md rounded shadow-sm hover:opacity-90 transition-opacity flex items-center gap-2"
            >
      ...
      <div className="bento-grid">
        ...
        >
          <div className="flex-shrink-0 h-32 w-32 rounded-full border-4 border-surface-container-high p-1">
            <ImagePlaceholder
              icon="person"
              alt={`${currentMember.firstName}'s profile photo`}
              shape="circle"
              className="h-full w-full text-[40px]"
            />
          </div>
          <div className="flex-grow">
            <div className="inline-flex items-center px-3 py-1 bg-secondary-container text-on-secondary-container rounded-full text-label-sm font-label-sm mb-4">
        ...
        >
          ...
          <button
            type="button"
            className="bg-transparent text-on-primary-container mt-6 w-full py-3 border border-on-primary-container/40 rounded-lg font-label-md text-label-md hover:bg-on-primary-container/10 transition-colors"
          >
            Add to Apple Wallet
          </button>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {quickBenefits.map((benefit) => (
              <div
                key={benefit.title}
                className="p-6 bg-surface-container-lowest border border-outline-variant rounded-xl hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 cursor-pointer"
              >
        ...
        >
          <div className="flex justify-between items-center mb-6">
            ...
            <a className="font-label-sm text-label-sm text-primary hover:underline" href="#activity">
              View All
            </a>
          </div>
          <div className="space-y-6">
            {recentActivity.map((activity) => (
              <div key={activity.title} className="flex gap-4">
                ...
                <div>
                  <p className="font-label-md text-label-md text-primary">{activity.title}</p>
                  <p className="font-label-sm text-label-sm text-secondary">{activity.meta}</p>
                </div>
              </div>
            ))}
            ...
            <div className="mt-8 pt-8 border-t border-outline-variant">
              ...
              <div className="bg-surface-container-lowest p-4 rounded-lg border border-outline-variant flex items-center gap-4">
                <div className="text-center px-3 border-r border-outline-variant">
                  <p className="text-headline-md text-primary font-bold">{upcomingEvent.day}</p>
                  <p className="text-label-sm text-secondary font-label-sm">{upcomingEvent.month}</p>
                </div>
                <div>
                  <p className="font-label-md text-label-md text-primary">{upcomingEvent.title}</p>
                  <p className="font-label-sm text-label-sm text-secondary">{upcomingEvent.location}</p>
                </div>
              </div>
            </div>
          </div>
